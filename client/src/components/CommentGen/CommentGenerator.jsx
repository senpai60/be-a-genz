...
function ButtonPrimary({ children, handleClick, selectionStyle = "", disabled = false, Icon, type = "button" }) {
  return (
    <button
      type={type}
      onClick={handleClick}
  ...
  if (!generatedComment) {
    return (
      <div className="p-6 bg-zinc-800 rounded-xl shadow-lg h-full flex flex-col justify-center items-center">
        ...
        <p className="text-zinc-400 text-center">Your generated comment will appear here.</p>
      </div>
    );
  }
...
const CommentGenerator = ({ backendUri = '/prompt/generate-comment' }) => {
  ...
  const fetchWithBackoff = useCallback(async (url, options) => {
    let lastError = null;
    for (let i = 0; i < 4; i++) { // Max 4 attempts
      try {
  ...
  const generateComment = async (e) => {
    ...
    // --- System instruction updated to handle optional image ---
    const systemPrompt = `You are a creative social media comment generator. Based on the user's provided context, an optional image, and desired tone, generate a single, compelling, and short comment (max 2 sentences). The comment must match the requested tone: ${commentTone}. If an image is provided, base the comment on the image's content as well.`;

    const userQuery = `Post context: "${postContext}"`;
    ...
    // Using gemini-2.5-flash-preview-09-2025 (which supports vision)
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;

    const payload = {
        contents: [{ parts: parts }], // Use the dynamic parts array
        systemInstruction: {
    ...
    try {
        ...
        const text = result.candidates?.[0]?.content?.parts?.[0]?.text || 'Could not generate a comment. Please try again.';
        setGeneratedComment(text);

    } catch (err) {
      console.error("Gemini API Request Failed:", err);
      setError("Failed to connect to the generator. Please check your network or try a different context.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <section className='md:p-4 w-full h-full text-zinc-100 bg-zinc-950'>
      <h1 className='text-3xl sm:text-4xl tracking-wide mb-6'>
        Instant Comment Generator <Zap className='w-6 h-6 inline-block text-yellow-400 ml-2'/>
      </h1>
      ...
      <div className="main flex flex-col lg:flex-row gap-8 items-stretch w-full h-[calc(100%-100px)]">
        ...
        <div className="left w-full lg:w-1/2 p-6 bg-zinc-800 rounded-xl shadow-lg overflow-y-auto">
          <h2 className='text-xl font-semibold text-teal-300 mb-4'>1. Context & Tone</h2>
          ...
          <form onSubmit={generateComment} className="space-y-5">
            ...
            <div className="form-group">
              ...
              <textarea
                id="context"
                value={postContext}
                onChange={(e) => { setPostContext(e.target.value); setError(null); }}
                rows="6"
            ...
            <div className="form-group">
              ...
              >
                {tones.map(tone => (
                  <option key={tone.value} value={tone.value} className="bg-zinc-800 text-zinc-100">
                    {tone.label}
                  </option>
                ))}
              </select>
            ...
            <div className="form-group">
              ...
              {imagePreviewUrl ? (
                ...
                <div className="mt-2 relative w-full max-w-xs">
                  <img src={imagePreviewUrl} alt="Selected preview" className="rounded-lg object-cover w-full max-h-64 shadow-md" />
                  <button
                    type="button"
                    onClick={clearImage}
                ...
                <label
                  ...
                  className={`
                    ...
                    ${isLoading ? 'cursor-not-allowed opacity-60' : 'cursor-pointer hover:border-teal-500 hover:text-teal-400 transition-colors'}
                  `}
                >
                  {/* Inline Image SVG */}
            ...
            {/* ERROR MESSAGE */}
            {error && (
              <p className="text-red-500 text-sm mt-2 p-2 bg-red-900/30 border border-red-700 rounded-lg">{error}</p>
            )}
            ...
            <div className="pt-4">
              ...
              {isLoading && <span className='ml-4 text-zinc-400 text-sm'>May take a few moments...</span>}
            </div>
          </form>
        </div>
